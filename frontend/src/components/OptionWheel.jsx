...
const OptionWheel = ({
  ...
  const runFrame = useCallback(now => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      el.style.transform = `translate3d(${x.toFixed(2)}px, ${y.toFixed(2)}px, 0px) translateY(-50%) rotate(${rot.toFixed(3)}deg)`;
  ...
  useEffect(() => {
    ...
    const onWheel = e => {
      if (cfgRef.current.preventWheel) {
        e.preventDefault();
      }
      const cfg = cfgRef.current;
    ...
    el.addEventListener('wheel', onWheel, { passive: !cfgRef.current.preventWheel });
    return () => {
    ...
    <div
      ...
      {items.map((label, index) => (
        <div
          ...
          className={`absolute top-1/2 cursor-pointer whitespace-nowrap leading-none transition-colors duration-150 will-change-[transform,opacity,filter] ${
